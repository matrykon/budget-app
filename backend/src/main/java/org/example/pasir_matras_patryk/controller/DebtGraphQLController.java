...
import org.example.pasir_matras_patryk.dto.DebtDTO;
import org.example.pasir_matras_patryk.model.Debt;
import org.example.pasir_matras_patryk.model.User;
import org.example.pasir_matras_patryk.service.DebtService;
        ...
        return debtService.getGroupDebts(groupId).stream()
                .peek(debt -> {
                    if (debt.getTitle() == null) {
                        debt.setTitle("Brak opisu"); // Ustawiamy bezpieczny tytuł zamiast null
                    }
                })
                .toList();
    }
